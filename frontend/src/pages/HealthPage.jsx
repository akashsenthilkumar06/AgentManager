...
function findWorkspaceTarget(result, architecture) {
  ...
  if (result.kind === "agent") {
    ...
    return agent ? { agentId: agent.id, toolName: null } : null;
  }
  ...
  return agent ? { agentId: agent.id, toolName: tool.name } : null;
}
...
export default function HealthPage({ health, architecture, setHealth, notify }) {
  ...
  function openInWorkspace(result) {
    ...
    if (target.toolName) params.set("tool", target.toolName);
