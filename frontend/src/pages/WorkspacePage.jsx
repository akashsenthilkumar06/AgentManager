...
function Verification({ verification, contextUsed, toolCalls }) {
  ...
  return <div className={`verification-card ${open ? "open" : ""}`}>
    <button className="verification-summary" onClick={() => setOpen((value) => !value)}>
      ...
      <span><strong>Output {verification.status}</strong><small>{Math.round(verification.confidence * 100)}% confidence · {toolCalls.length} tool used</small></span>
      <b>{open ? "Hide evidence" : "See evidence"}</b>
    </button>
    {open && <div className="verification-details"><p>{verification.summary}</p><div className="proof-columns"><div><span>Acceptance criteria</span>{verification.criteria.map((item) => <small key={item}>✓ {item}</small>)}</div><div><span>Grounding evidence</span>{verification.evidence.map((item) => <small key={item}>↳ {item}</small>)}</div></div><div className="context-receipt"><span>Context used</span>{contextUsed.map((item) => <em key={item}>{item}</em>)}</div></div>}
  </div>;
}
...
function ManagerWorkspace({ agent, onRefresh, notify }) {
  ...
  return <div className={`manager-workspace ${liveOpen ? "live-open" : "live-closed"}`}>
    ...
    <section className="manager-chat"><header className="manager-chat-header"><div><span className="manager-presence"><i />Manager Agent</span><small>Working on {agent.name}</small></div><button className={`live-work-toggle ${liveOpen ? "active" : ""}`} onClick={() => setLiveOpen((open) => !open)}><span>⌁</span> Live work{latestManagerMessage?.actions.length ? <b>{latestManagerMessage.actions.length}</b> : null}<i>{liveOpen ? "›" : "‹"}</i></button></header>
      <div className="manager-chat-scroll">{conversation ? <div className="manager-message-list">{conversation.messages.map((message) => <ManagerMessage key={message.id} message={message} onApply={applyChanges} applying={applying} />)}{working && <div className="manager-thinking"><span><i /><i /><i /></span>Manager is selecting tools and inspecting the agent…</div>}<div ref={bottomRef} /></div> : <div className="manager-welcome"><span className="manager-orb">M</span><p className="eyebrow">YOUR AGENTIC MANAGER</p><h2>What should we change about {agent.name}?</h2><p>Describe the outcome. The Manager will choose the right MCP specialists, inspect the client workspace, make the change, and validate it.</p><div className="manager-starters">{EDIT_STARTERS.map((starter) => <button key={starter} onClick={() => send(null, starter)}>{starter}<span>→</span></button>)}</div></div>}</div>
      <form className="manager-composer" onSubmit={send}><textarea rows="2" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder={`Tell the Manager what to change in ${agent.name}…`} /><div><span><i />{autonomy === "auto" ? "Validated changes apply automatically" : "Changes wait for review"}</span><div className="manager-compose-actions"><div className="composer-permission"><span>Permission</span><button type="button" className={autonomy === "review" ? "active" : ""} onClick={() => setAutonomy("review")}>Review</button><button type="button" className={autonomy === "auto" ? "active" : ""} onClick={() => setAutonomy("auto")}>Auto</button></div><button className="manager-send-button" disabled={working || !prompt.trim()}>{working ? "Working…" : "Send"} <b>↑</b></button></div></div></form>
    </section>
